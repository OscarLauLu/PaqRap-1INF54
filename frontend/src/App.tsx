...
export const App: React.FC = () => {
  ...
  // "Iniciar Simulación" en Configuración: arranca el escenario elegido y abre su pantalla
  const iniciarDesdeConfiguracion = async () => {
  ...
  const enConfiguracion = activeSection === 'configuracion';
  const enReportes = activeSection === 'reportes';
  const enSimulacion = activeSection === 'simulacion';

  const titulo = enRegistro
    ? 'Registro'
    : enConfiguracion
    ? 'Configuración de la simulación'
    : enReportes
    ? 'Reportes'
    : NOMBRE_ESCENARIO[escenarioSel];
    ...
    <div className="flex min-h-screen bg-[#f8fafc] text-gray-800">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Header
          ...
          showFinalizar={enSimulacion && escenarioSel === 'SIMULACION_5D'}
        />
        ...
        {enRegistro ? (
          ...
          <ConfiguracionPage
            ...
            onHoraInicio={setHoraInicio}
            motorCorriendo={motorCorriendo}
            ocupadoPor={ocupadoPor}
            onIniciar={iniciarDesdeConfiguracion}
            onDetener={terminar}
          />
        ) : enReportes ? (
          <PantallaPendiente titulo="Reportes" detalle="Esta pantalla todavía no está desarrollada." />
        ...
        <main className="flex-1 p-6 md:p-8 flex flex-col w-full h-[calc(100vh-72px)] overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-4 text-sm font-semibold text-gray-600 bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
            <div className="flex items-center space-x-8 px-4">
              <div>Fecha: <span className="text-gray-900 ml-1">{sinDatos ? '--' : formatFecha(reloj.instanteActual)}</span></div>
              <div className="flex items-center"><ClockIcon className="w-4 h-4 mr-1"/> Tiempo transcurrido: <span className="text-gray-900 ml-1">{sinDatos ? '--' : getTranscurrido()}</span></div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <button onClick={() => arrancar('SIMULACION_5D')} disabled={motorCorriendo} className="p-2 bg-blue-50 text-blue-600 rounded-lg border border-blue-100 hover:bg-blue-100 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"><Play size={18} fill="currentColor" /></button>
              <button onClick={pausar} className="p-2 bg-gray-50 text-gray-600 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors"><Pause size={18} fill="currentColor" /></button>
              <button onClick={terminar} className="p-2 bg-gray-50 text-gray-600 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors"><Square size={18} fill="currentColor" /></button>
          ...
          <div className="flex flex-1 gap-6 min-h-0">
            ...
            <div className="flex-1 h-full flex flex-col">
              <MapaOperaciones almacenes={almacenes} unidades={sinDatos ? [] : unidades} pedidos={sinDatos ? [] : pedidos} selectedUnidadId={selectedUnidadId} onSelectUnidad={setSelectedUnidadId} />
            </div>

            {/* Right Panel (Fixed Width) */}
            <RightPanel reloj={reloj} almacenes={sinDatos ? [] : almacenes} unidades={sinDatos ? [] : unidades} pedidos={sinDatos ? [] : pedidos} selectedUnidadId={selectedUnidadId} />
          </div>
