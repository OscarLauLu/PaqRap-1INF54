...
const TITULO_ESCENARIO: Record<TipoEscenario, string> = {
  DIA_A_DIA: 'Operación día a día',
  SIMULACION_5D: 'Simulación 5D',
  COLAPSO_LOGISTICO: 'Simulación hasta el colapso',
};

export const App: React.FC = () => {
  const [activeSection, setActiveSection] = useState<Seccion>('configuracion');
  const [escenarioActivo, setEscenarioActivo] = useState<TipoEscenario>('DIA_A_DIA');
  ...
  const titulos: Record<Seccion, { title: string; subtitle?: string }> = {
    ...
    simulacion: { title: TITULO_ESCENARIO[escenarioActivo] },
  ...
  return (
    <div className="flex min-h-screen bg-(--color-surface-50) text-(--color-ink-900)">
      <Sidebar activeSection={activeSection} onSelectSection={cambiarSeccion} />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {activeSection === 'configuracion' && <ConfiguracionPage onSimulacionIniciada={irASimulacion} />}
          ...
          <main className="flex-1 p-6 md:p-8 flex flex-col w-full h-[calc(100vh-72px)] overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-4 text-sm font-semibold text-(--color-ink-500) bg-white p-3 rounded-xl border border-(--color-line-200) shadow-sm">
              <div className="flex items-center space-x-8 px-4">
                <div>Fecha: <span className="text-(--color-ink-900) ml-1">{formatFecha(reloj.instanteActual)}</span></div>
                <div className="flex items-center"><ClockIcon className="w-4 h-4 mr-1" /> Tiempo transcurrido: <span className="text-(--color-ink-900) ml-1">{getTranscurrido()}</span></div>
                <div>Día: <span className="text-(--color-ink-900) ml-1">{diaActual}</span></div>
              </div>

              <div className="flex items-center space-x-2">
                <button onClick={() => iniciar(escenarioActivo)} className="p-2 bg-(--color-brand-50) text-(--color-brand-500) rounded-lg border border-(--color-brand-100) hover:bg-(--color-brand-100) transition-colors"><Play size={18} fill="currentColor" /></button>
                <button onClick={detener} className="p-2 bg-gray-50 text-(--color-ink-500) rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors"><Pause size={18} fill="currentColor" /></button>
                <button onClick={finalizarSimulacion} className="p-2 bg-gray-50 text-(--color-ink-500) rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors"><Square size={18} fill="currentColor" /></button>
